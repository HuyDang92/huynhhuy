...
export function Sudo() {
   ...
   return (
      <>
         ...
         />
         {/* <a.mesh geometry={(nodes as any).SudoHead.geometry} material={(nodes as any).SudoHead.material} position={[0.68, 0.33, -0.67]} {...spring} /> */}
      </>
   );
}
...
export function Camera() {
   ...
   return (
      <a.group position={[-0.58, 0.83, -0.03]} rotation={[Math.PI / 2, 0, 0.47]} {...spring}>
         <mesh geometry={(nodes as any).Camera.geometry} material={(nodes as any).Camera.material} />
         <mesh geometry={(nodes as any).Camera_1.geometry} material={materials.Lens} />
      </a.group>
   );
}
...
export function Cactus() {
   const { nodes, materials } = useGLTF("/level-react-draco.glb");
   return (
      <mesh geometry={(nodes as any).Cactus.geometry} position={[-0.42, 0.51, -0.62]} rotation={[Math.PI / 2, 0, 0]}>
         <MeshWobbleMaterial factor={0.4} map={(materials as any).Cactus.map} />
      </mesh>
   );
}
