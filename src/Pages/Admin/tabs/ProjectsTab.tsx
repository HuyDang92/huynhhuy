...
function ProjectCard({
  ...
  return (
    <div className="bg-gray-50 border border-gray-300 rounded-2xl p-5 space-y-4">
      ...
      <div>
        <label className="text-gray-600 text-xs mb-1 block">Thumbnail URL</label>
      ...
      <div>
        <label className="text-gray-600 text-xs mb-1 block">Role</label>
        <textarea rows={2} value={item.role} onChange={(e) => set("role", e.target.value)}
          className="w-full bg-white border border-gray-300 rounded-xl px-3 py-2 text-gray-900 text-sm focus:outline-none focus:border-[#FF5858]/60 transition-colors resize-y" />
      </div>
      <div>
        <label className="text-gray-600 text-xs mb-1 block">Technologies</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {item.type === "web" && (
          <div>
            <label className="text-gray-600 text-xs mb-1 block">Link</label>
      ...
      {item.type === "app" && (
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-gray-600 text-xs mb-1 block">iOS Link</label>
            <input type="text" value={item.ios ?? ""} onChange={(e) => set("ios", e.target.value)}
              className="w-full bg-white border border-gray-300 rounded-xl px-3 py-2 text-gray-900 text-sm focus:outline-none focus:border-[#FF5858]/60 transition-colors" />
          </div>
          <div>
            <label className="text-gray-600 text-xs mb-1 block">Android Link</label>
  ...
  return (
    <div className="w-full space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="text-gray-900 text-xl font-semibold">Projects</h2>
        <button onClick={addItem} className="border border-[#FF5858]/40 hover:bg-[#FF5858]/10 text-[#FF5858] rounded-xl px-4 py-2 text-sm font-bold transition-colors">
          + Add
        </button>
      </div>
